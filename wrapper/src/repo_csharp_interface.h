...
extern "C"
{
	...
	*/
		...
		char* password)
	{
		...
		return wrapper->connect(address, port, username, password);
	}
	...
	*/
	...
	{
		auto wrapper = repo::lib::CSharpWrapper::getInstance();
		wrapper->getMaterialInfo(materialID, diffuse, specular, shininess);
	}
	...
	*/
	...
	{
		auto wrapper = repo::lib::CSharpWrapper::getInstance();		
		return cStringCopy(wrapper->getSuperMeshInfo(index, subMeshes, numVertices, numFaces, hasNormals, hasUV));
	}
