...
const static int MAJOR_NUMBER = 2;
const static int MINOR_NUMBER = 5;
const static int MINOR_REV = 10;
...
{
	std::string msg;
	token = controller->authenticateToAdminDatabaseMongo(msg, address, port, username, password);
	return token;
}
	...
	float* &shininess) const
{
	...
	if (it != matNodes.end())
	{
		...
		bool hasDiffuse = it->second.diffuse.size();
		bool hasSpecular = it->second.specular.size();
		for (int i = 0; i < 4; ++i)
		{
			if (hasDiffuse)
		...
		if (it->second.opacity == it->second.opacity)
	...
	const std::string &revisionID)
{
	...
	if(revisionID.empty())
		scene = controller->fetchScene(token, database, project, REPO_HISTORY_MASTER_BRANCH, true, false);
	else
		scene = controller->fetchScene(token, database, project, revisionID, false, false);
	bool success  = false;
	if (scene)
	{
		projectName = project;
		...
		superMeshes = controller->initialiseAssetBuffer(token, scene, jsonFiles, serialisedFaceBuf, idMapBuf, meshMappings);
